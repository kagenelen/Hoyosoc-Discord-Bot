...
def new_guess(discord_id, bet, given_attempts):
	...
	if given_attempts > max_attempts:
		return "The maximum allowed guesses for this bet is " + str(max_attempts) + "."
	...
	return "You have 10 minutes to guess the correct number between 0 and " + str(bet) + " and earn " + str(winnings) + " primojems."
...
def make_guess(discord_id, guess):
	...
	if session["used_attempts"] == session["allowed_attempts"]:
		res = "Out of attempts. The correct number is " + str(session["target"]) + "."
		data.pop(discord_id)
...
def coinflip(discord_id, coin_amount, head_amount, bet):
	...
	if flip_result.count("H") == head_amount:
		payout = int(bet * (1 / (math.comb(coin_amount, head_amount) / pow(2, coin_amount))))
