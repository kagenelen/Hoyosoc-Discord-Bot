...
def runbot():
  ...
  async def on_message(message):
    ...
    if (message.channel.id == VERIFICATION_CHANNEL):
      ...
      character_message = "*" + data.get(
        welcome_character)[1] + "    " + data.get(welcome_character)[0] + "*"
  ...
  async def daily_role_expiry_check():
    ...
    for e in expired:
      ...
      role = discord.utils.get(gensoc_guild.roles, name=e[1][0].capitalize())
  ...
  async def set_verification(interaction, verify_channel: str):
    ...
    await interaction.response.send_message(
      "Verification channel has been set to " + verify_channel, ephemeral=True)
    data = helper.read_file("config.json")
  ...
  async def add_uid(interaction, uid: str):
    ...
    if result == False:
      ...
      await interaction.response.send_message(str(uid) + " has been added.")
  ...
  async def remove_uid(interaction, uid: str):
    ...
    if result == False:
      ...
      await interaction.response.send_message(str(uid) + " has been removed.",
                                              ephemeral=True)
  ...
  async def find_uid(interaction, target_user: discord.Member):
    ...
    if result == False:
      await interaction.response.send_message(target_user.name +
                                              " does not have any uid saved.")
    else:
      await interaction.response.send_message(target_user.name +
                                              " has the following uid: " +
                                              result)
  ...
  async def start_bet(interaction, bracket_id: str, candidates: str,
    ...
    await interaction.response.send_message(bracket_id + " has been created.")
  ...
  async def payout_bet(interaction, bracket_id: str, winning_candidate: str):
    ...
    await interaction.response.send_message("Earnings for " +
                                            winning_candidate +
                                            " bets in bracket " + bracket_id +
                                            " has been sent.")
  ...
  async def make_bet(interaction, bracket_id: str, candidate: str,
    ...
    if res != None:
      ...
      await interaction.response.send_message("Successfully made a bet of " +
                                              str(amount) + " for " +
                                              candidate.lower(),
                                              ephemeral=True)
  ...
  async def my_bets(interaction):
    ...
    for r in range(0, len(res)):
      embed.add_field(name=res[r][0].title(),
                      value="Candidate: " + res[r][2].title() + " | Amount: " +
                      str(res[r][1]),
                      inline=False)
  ...
  async def ongoing_bets(interaction):
    ...
    # candidates
    for r in range(0, len(res)):
      embed.add_field(name=res[r][0].title() + " | Ending: " + res[r][2],
                      value=res[r][1],
                      inline=False)
  ...
  async def checkin(interaction):
    ...
    if res == None:
      ...
      await interaction.response.send_message(
        "You got " + str(res[0]) +
        " primojems from the check in. Current streak: " + str(res[1]) + ".")
  ...
  async def view_shop(interaction):
    ...
    description = ("7 day role costs 1500\n" + "30 day role costs 4500\n" +
                   "Buying a role you own will increase the duration.\n\n")
    if helper.is_booster(interaction.user):
      description += "Server booster discount: 50%\n"
  ...
  @tree.command(name="buy",
                ...
                guild=discord.Object(id=GENSOC_SERVER))
  async def buy_item(interaction, item_name: str, duration: int):
    booster = helper.is_booster(interaction.user)
    res = gambling.buy_role(interaction.user.id, item_name, duration, booster)

    if res != None:
      await interaction.response.send_message(res, ephemeral=True)
    else:
      await interaction.response.send_message(
        "Successfully bought " + item_name.lower() + " for " + str(duration) +
        " days. Use **/equip** to use the role.")
  ...
  async def equip_role(interaction, role_name: str):
    if gambling.is_role_owned(interaction.user.id, role_name):
      ...
      if role in interaction.user.roles:
        # Unequip role
        await interaction.user.remove_roles(role, reason="Unequip shop role.")
        await interaction.response.send_message("You have unequipped " +
                                                role_name.lower() + ".")
      else:
        # Equip role
        await interaction.user.add_roles(role, reason="Equip shop role.")
  ...
  @tree.command(name="payout_attendance",
  ...
  async def payout_attendance(interaction, attendee_list: str, amount: int):
    ...
    res = gambling.update_user_list_currency(attendee_list, amount,
                                             interaction.guild)
    await interaction.response.send_message(
      str(amount) + " given to these users: " + ", ".join(res), ephemeral=True)

  ################### MINIGAMES ####################################################

  @tree.command(
    name="coinflip",
    description="Flip a number of coins and guess how many will land on head.",
    guild=discord.Object(id=GENSOC_SERVER))
  async def flip(interaction, coin_amount: int, head_amount: int, bet: int):
    res = minigame.coinflip(interaction.user.id, coin_amount, head_amount, bet)
  ...
  async def blackjack(interaction, bet: str):
    ...
    embed = discord.Embed(title=interaction.user.name + "\'s Blackjack Game",
